...
public:
    ...
    // Sets the cursorColumn value
    void setCol(unsigned int newCol);

    // Sets the cursorLine value
    void setRow(unsigned int newRow);

    // Sets the text displayed on a line
    void setText(std::string& newText, unsigned int row);

    // Adds a new line (by inserting an item into totalLines vector)
    void createNewLine(std::string& text, unsigned int row);

    // Deletes a line (by deleting an item into totalLines vector)
    void deleteLine(unsigned int row);
